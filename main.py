...
from core.baseline import BaselineCollector
...
class PinocchioApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.baseline = BaselineCollector()
        self.llm = LLMClient()
        self.tts = TTSEngine()
        ...
        self._current_emotion_kr = "보통 😐"

        # ── 음성 분석 결과 (최신) ──
        self._latest_voice_data = None
        self._latest_analysis = None
        self._latest_face_data = None
    ...
    def _update_frame(self):
        ...
        if face_data.detected:
            ...
            iris_instability = self.vision.get_iris_instability()
            self.iris_ratio_avg = (
                face_data.iris_ratio_left + face_data.iris_ratio_right
            ) / 2.0
            self.gaze_direction = face_data.gaze_direction

            # ── 베이스라인 수집 (진행 중이면 샘플 추가) ──
            if not self.baseline.is_ready:
                self.baseline.add_vision_sample(face_data, self.blink_rate, iris_instability)

            # ── Phase 3: 멀티모달 분석 (영상만 매 프레임) ──
            ...
            self.stress_score = min(100, int(analysis.combined_score))
            self._current_emotion = analysis.emotion
            self._current_emotion_kr = analysis.emotion_kr
            self._latest_analysis = analysis
            self._latest_face_data = face_data
    ...
    def _update_score_display(self):
        ...
        # ── 감정 라벨 업데이트 ──
        self.emotion_label.setText(self._current_emotion_kr)
    ...
    def _update_indicators(self, face_data):
        ...
        # ── 음성 지표는 별도 메서드 (분석 완료 시만 갱신) ──

    def _update_voice_indicators(self):
        """음성 지표 패널 갱신 — 음성 분석 완료 직후 호출"""
        vd = self._latest_voice_data
        if not vd or vd.f0_mean <= 0:
            return
    ...
    def _start_first_dialogue(self):
        # ── 베이스라인 수집 시작 ──
        self.baseline.start(on_complete=self._on_baseline_complete)
    ...
    def _on_baseline_complete(self, vision_bl, voice_bl):
        """베이스라인 수집 완료 콜백"""
        self.analyzer.set_baseline(vision_bl, voice_bl)
        print(f"[Main] 베이스라인 → Analyzer 전달 완료")
        ...
        if hasattr(self, '_pending_wav_path') and self._pending_wav_path:
            ...
            def _voice_analysis():
                try:
                    ...
                    # 메인 스레드에서 UI 갱신 (QTimer.singleShot 사용)
                    QTimer.singleShot(0, self._update_voice_indicators)
                except Exception as e:
                    print(f"[Voice] 분석 실패: {e}")

            threading.Thread(target=_voice_analysis, daemon=True).start()

        # ── LLM 호출 (음성 분석과 동시 실행) ──
        prompt = self.llm.build_prompt(
            stress_score=self.stress_score,
            user_text=text,
            analysis_result=getattr(self, '_latest_analysis', None),
            face_data=getattr(self, '_latest_face_data', None),
            voice_data=self._latest_voice_data,
        )
